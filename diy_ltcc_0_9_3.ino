...
uint8_t ignVolts_gross;                 // ignVolts_f rounded to single byte integer
...
void setup() {
  ...
  sei();
  /* // blink the led to indicate system ready
...
void loop() {
  ...
  char serial_out[20];
    ...
    if ( lrTimeHi != 0 ) {
      ...
      if (sequenceIndex < 0) {
        ...
        dwellDegrees = getDwell(rpmTable[0] + 1, 0, ignVolts_gross, rpmIndex);
      } else {
        dwellDegrees = getDwell(rpmTimeHi, rpmTimeLast, ignVolts_gross, rpmIndex); // might want to consider doing this less freq at higher rpms
  ...
  if (logFlag) {
    ...
    #ifdef LOGGING
    if ( sequenceIndex < 0 ) {
      sprintf(serial_out,"R%d:S%d:D%d:C%d:V%d:E%d:L%d", rpmReference[rpmIndex], saDegrees, dwellDegrees, 0, ignVolts_gross, errorDegs, dwellLag);
    } else {
      sprintf(serial_out,"R%d:S%d:D%d:C%d:V%d:E%d:L%d", rpmReference[rpmIndex], saDegrees, dwellDegrees, sequencer[sequenceIndex].cylNo, ignVolts_gross, errorDegs, dwellLag);
    }
...
void populateDwellDegrees() {
  ...
  for (int i=0; i<RPM_DIVS; i++) {
    ...
    if (i < CRANK_RPM_DIVS) { // CRANK_DWELL_TGT
      ...
      dwellTable[i].dwell17V = int ((CRANK_DWELL_TGT + VOLT_COMP_17) * 1000) / (rpmTable[i] / 90);
      if (dwellTable[i].dwell17V == 0) dwellTable[i].dwell17V = 1;
    } else {
      ...
      dwellTable[i].dwell17V = int ((DWELL_TGT + VOLT_COMP_17) * 1000) / (rpmTable[i] / 90);
    }
    #ifdef SERIAL_DEBUG
  ...
  Serial.print('\t');
  for (int i=0; i<RPM_DIVS; i++) {
    Serial.print(dwellTable[i].dwell17V);
    Serial.print('\t');
  }
  Serial.println("");
  #endif
...
uint8_t getDwellAtVoltage(uint8_t dwellInd, uint8_t sysVolts) {
  ...
  switch (sysVolts) {
    ...
    case (17):
      return dwellTable[dwellInd].dwell17V;
      break;
  }  
  if (sysVolts < 8) return dwellTable[dwellInd].dwell8V;
  if (sysVolts > 17) return dwellTable[dwellInd].dwell17V;
